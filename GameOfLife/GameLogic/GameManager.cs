...
public class GameManager {
    ...
    private void KillSims() {
        for (int y = 0; y < _grid.Height; y++)
            for (int x = 0; x < _grid.Width; x++)
                foreach (ISimulable sim in _grid[x, y].Where(sim => sim.ShouldDie()))
                    _grid.RemoveSim(sim);
    }
    ...
    private void ReproduceSims() {
        for (int y = 0; y < _grid.Height; y++)
            for (int x = 0; x < _grid.Width; x++)
                foreach (ISimulable sim in _grid[x, y].Where(sim => sim.ShouldCreateDescendant(_grid)))
                    _grid.CreateSim(sim.NewDescendant(_grid));
    }

    private void MoveSims() {
        for (int y = 0; y < _grid.Height; y++)
            for (int x = 0; x < _grid.Width; x++)
                foreach (ISimulable sim in _grid[x, y].Where(sim => sim.NextPosition is not null)) {
